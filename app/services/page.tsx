...
export default async function ServicesPage() {
    ...
    // Map Prismic services data to match the format needed
    const services = homepage?.data.services.map((item: any, index: number) => {
        ...
        return {
            ...
            buttonTextColor: isDarkBg ? 'text-white' : 'text-dark', // White text on dark bg, dark text on light bg
        }
    }) || []

    // Hardcoded content for "What we offer" and "Benefits" sections
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
                        ...
                        <div className="flex flex-col justify-between h-full min-h-[200px]">
                            ...
                            <div className="flex flex-col">
                                <h3
                                    className={`text-lg font-semibold inline-block ${service.titleBgColor} px-2 py-1 rounded-lg w-fit`}
                                >
                                    {service.title}
                                </h3>
                                {service.subtitle && (
                                    <h3 className={`text-lg font-semibold inline-block ${service.titleBgColor} px-2 py-1 rounded-lg w-fit`}>{service.subtitle}</h3>
                                )}
                            </div>
